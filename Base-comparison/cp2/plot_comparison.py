...
def main():
    ...
    # -----------------------------------------------------------------------
    # Row 1, col 1: Comparable collapse-risk score (both runs)
    # -----------------------------------------------------------------------
    # Use the same entropy-derived risk metric for BOTH runs so the panel
    # remains a direct apples-to-apples comparison.  Risk is the inverse of
    # normalized router entropy: 0 = healthy, 1 = fully collapsed.
    ax = axes[1, 1]
    layer_colors = ["#1f77b4", "#ff7f0e", "#9467bd"]
    for gate, color in zip(GATES, layer_colors):
        without_risk = [float(np.clip(1.0 - e, 0.0, 1.0)) for e in without["entropy"][gate]]
        with_risk    = [float(np.clip(1.0 - e, 0.0, 1.0)) for e in with_["entropy"][gate]]

        ax.plot(without["step"], without_risk,
                label=f"layer {gate.split('.')[1]} — Without MoEWatch",
                linewidth=1.0, color=color, linestyle="--", alpha=0.8)
        ax.plot(with_["step"], with_risk,
    ...
    ax.set_title("Collapse risk score — all layers\n"
                 "same entropy-derived risk metric in both runs")
    ax.set_xlabel("step")
